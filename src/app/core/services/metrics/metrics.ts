  ...
  async getCallRecordsByMonth(year: number, month: number): Promise<CallRecord[]> {
    ...
    for (let from = 0; ; from += pageSize) {
      const { data, error } = await this.supabase.client
        .from('call_records')
        .select(
          'id,user_id,work_date,is_technical_visit,is_rescheduled,is_installation,technical_visit_count,regular_visit_count,installation_visit_count,rescheduled_visit_count,is_transferred,transfer_area,notes,created_at,updated_at',
        )
        .gte('work_date', start)
        .lt('work_date', end)
        .order('work_date', { ascending: true })
